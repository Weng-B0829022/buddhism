...
class StoryboardManager:
    def __init__(self, file_path, random_id, initial_storyboard=None, avatar_count=2):
        self.avatar_count = avatar_count
        ...
        if "storyboard" in initial_storyboard:
            for i, paragraph in enumerate(initial_storyboard["storyboard"]):
                paragraph["needAvatar"] = i < self.avatar_count  # True for first two paragraphs, False for others
            ...
            try:
                ...
                if isinstance(data, dict) and "storyboard" in data:
                    ...
                    for i, paragraph in enumerate(data["storyboard"]):
                        paragraph["needAvatar"] = i < self.avatar_count
                    return data
                else:
    ...
    def _update_paragraph(self, paragraph_index, new_data):
        ...
        if paragraph_index < len(self.storyboard["storyboard"]):
            ...
            new_paragraph = {
                ...
                "voiceover": "",
                "characterCount": 0,
                "needAvatar": paragraph_index < self.avatar_count  # True for first two paragraphs, False for others
            }
            new_paragraph.update(new_data)
            self.storyboard["storyboard"].append(new_paragraph)
        
        # 確保 needAvatar 規則始終被遵守
        self.storyboard["storyboard"][paragraph_index]["needAvatar"] = paragraph_index < self.avatar_count
    ...
    def set_image_config(self,image_name, config_item, extend_image_name, extend_config_item = None):
        self.img_config =  {
            "img_path": image_name,
        ...
        if extend_config_item:
            self.extend_img_config =  {
                "img_path": extend_image_name,
                "top_left": extend_config_item['top_left'],
                "top_right": extend_config_item['top_right'],
                "bottom_right": extend_config_item['bottom_right'],
                "bottom_left": extend_config_item['bottom_left'],
                "z_index": extend_config_item['z_index']
            }
    def add_config_to_all_paragraphs(self, is_background=False):
        for idx, paragraph in enumerate(self.storyboard["storyboard"]):
            # Ensure 'images' key exists and is a list
            if "images" not in paragraph:
                paragraph["images"] = []

            if is_background:
                if idx < self.avatar_count:
                    paragraph["images"].insert(0, self.img_config)
                else:
                    paragraph["images"].insert(0, self.extend_img_config)
            else:
                paragraph["images"].insert(0, self.img_config)
